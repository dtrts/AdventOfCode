...
// Make a map that is ingredient.allergen.amount of times.
// Make a map for however many times the allergen appears (this can't change)
// If the ingredient
...
func main() {
	...
	// input = inputTest
	allergenCount, ingredientCount, allergenIngredientCount, ingredientAllergenCount := parseInput(input)
	fmt.Println(ingredientCount)
	// GO through and remove where the ingredients don't appear enough for the allergens
	count := 0
	for ingredientToRemove, allergen := findRemoveIngredientFromAllergen(allergenCount, allergenIngredientCount); ingredientToRemove != ""; ingredientToRemove, allergen = findRemoveIngredientFromAllergen(allergenCount, allergenIngredientCount) {
		// fmt.Println("Removing ingredient", ingredientToRemove, "from allergen", allergen)
		// ingredientCount[ingredientToRemove] -= allergenIngredientCount[allergen][ingredientToRemove]
		// ingredientAllergenCount[ingredientToRemove][allergen]--
		allergenIngredientCount[allergen][ingredientToRemove] = 0
		count++
	}
	cleanMapMap(ingredientAllergenCount)
	cleanMapMap(allergenIngredientCount)
	// printMaps()

	fmt.Println(allergenIngredientCount)
	cleanAllergenIngredients(allergenIngredientCount)
	fmt.Println(allergenIngredientCount)

	matchedAllergensEncoded := getMatchedAllergensEncoded(allergenIngredientCount)
	fmt.Println(matchedAllergensEncoded)

	part1 := 0
	for ingredient, count := range ingredientCount {
		if _, exists := matchedAllergensEncoded[ingredient]; !exists {
			part1 += count
		}
	}
	fmt.Println(part1)

	// maps aren't alphabetical ???
	part2 := ""

	order := [][2]string{}
	for allergen, ingredientCountByAllergen := range allergenIngredientCount {
		for ingredient := range ingredientCountByAllergen {
			order = append(order, [2]string{allergen, ingredient})
			fmt.Println(allergen, ingredient)
		}
	}
	sort.Slice(order, func(i, j int) bool { return order[i][0] < order[j][0] })
	...
	fmt.Println(part2, order)
	// Clean map of maps

	// Go through the allergenIngredients, is allergenIngredients < allegenCount then remove

}

func getMatchedAllergensEncoded(allergenIngredientCount map[string]map[string]int) map[string]bool {
	allergensEncoded := make(map[string]bool)
	for _, ingredientCountByAllergen := range allergenIngredientCount {
		for ingredient := range ingredientCountByAllergen {
			allergensEncoded[ingredient] = true
		}
	}

	return allergensEncoded
}

func cleanAllergenIngredients(allergenIngredientCount map[string]map[string]int) {
	for unmatchedAllergens(allergenIngredientCount) {
		for allergen, ingredientCount := range allergenIngredientCount {
			if len(ingredientCount) == 1 {
				for ingredient := range ingredientCount {
					removeIngredientFromAllergenIngredients(allergenIngredientCount, ingredient, allergen)
				}
			}
		}
	}
...
func unmatchedAllergens(allergenIngredientCount map[string]map[string]int) bool {
	for _, oneAllergenIngredientCount := range allergenIngredientCount {
		if len(oneAllergenIngredientCount) > 1 {
			return true
		}
	}
	return false
...
func findRemoveIngredientFromAllergen(allergenCount map[string]int, allergenIngredientCount map[string]map[string]int) (string, string) {
	for allergen, oneAllIngredientCount := range allergenIngredientCount {
		for ingredient, count := range oneAllIngredientCount {
			if 0 < count && count < allergenCount[allergen] {
...
func removeIngredientFromAllergenIngredients(allergenIngredientCount map[string]map[string]int, ingredientDelete string, protectAllergen string) {
	for allergen, ingredientCount := range allergenIngredientCount {
		if allergen == protectAllergen {
			continue
		}
		delete(ingredientCount, ingredientDelete)
	}
	return
}

func printMaps(allergenCount map[string]int, ingredientCount map[string]int, allergenIngredientCount map[string]map[string]int, ingredientAllergenCount map[string]map[string]int) {
	fmt.Println("allergenCount")
	fmt.Println(allergenCount)
	fmt.Println("")
	fmt.Println("ingredientCount")
	fmt.Println(ingredientCount)
	...
	fmt.Println(allergenIngredientCount)
	fmt.Println("")
	fmt.Println("ingredientAllergenCount")
	fmt.Println(ingredientAllergenCount)
	fmt.Println("")
	fmt.Println("")
	return
...
func cleanMapMap(mapMap map[string]map[string]int) {
	for _, innerMap := range mapMap {
...
func parseInput(input string) (map[string]int, map[string]int, map[string]map[string]int, map[string]map[string]int) {

	allergenCount := make(map[string]int)
	ingredientCount := make(map[string]int)
	ingredientAllergenCount := make(map[string]map[string]int)
	allergenIngredientCount := make(map[string]map[string]int)

	for _, line := range strings.Split(input, "\n") {
		re := regexp.MustCompile(`^([a-z ]+) \(contains ([a-z ,]+)\)$`)
		matches := re.FindAllStringSubmatch(line, -1)
		ingredients := strings.Split(matches[0][1], " ")
		allergens := strings.Split(matches[0][2], ", ")
