...
func calcStringLengths(s string) (memoryLength int) {
	//assuming all characters are a single byte
	i := 0
	for i < len(s) {
		if i == 0 {
			i++
...
func encodeString(s string) (encondedString string) {
	encondedString += "\""
...
func main() {
	...
	// fmt.Println(inputSplit[2], calcStringLengths(inputSplit[2]))
	//Part1
	totalLength, memoryLength := 0, 0
	for _, v := range inputSplit {
		totalLength += len(v)
		memoryLength += calcStringLengths(v)
	}
	fmt.Println(totalLength - memoryLength)

	//Part2
	encodedLength := 0
	for _, v := range inputSplit {
		encodedLength += len(encodeString(v))
	}
	fmt.Println(encodedLength - totalLength)
