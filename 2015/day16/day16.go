...
func main() {
	...
	inputSplit := strings.Split(string(inputRaw), "\n")
	fmt.Println(inputSplit)

	//Create testResult collections
	goalResults := []testResult{testResult{"children", 3}, testResult{"cats", 7}, testResult{"samoyeds", 2}, testResult{"pomeranians", 3}, testResult{"akitas", 0}, testResult{"vizslas", 0}, testResult{"goldfish", 5}, testResult{"trees", 3}, testResult{"cars", 2}, testResult{"perfumes", 1}}

	//proces inputs

	sueNum := 0
	test0, test1, test2 := "", "", ""
	result0, result1, result2 := 0, 0, 0

	for _, v := range inputSplit {

		isMatchy := true

		fmt.Sscanf(v, "Sue %d: %s %d, %s %d, %s %d", &sueNum, &test0, &result0, &test1, &result1, &test2, &result2)
		test0 = strings.TrimSuffix(test0, ":")
		test1 = strings.TrimSuffix(test1, ":")
		test2 = strings.TrimSuffix(test2, ":")

		for _, v := range goalResults {
			if test0 == v.test && result0 != v.value {
				isMatchy = false
			}
			if test1 == v.test && result1 != v.value {
				isMatchy = false
			}
			if test2 == v.test && result2 != v.value {
				isMatchy = false
			}
		}
		if isMatchy == true {
		...
		// fmt.Println(v, sueNum, test0, test1, test2, result0, result1, result2)
		/*
		...
		*/
		...
		for _, v := range goalResults {
			...
			if test0 == v.test && result0 != v.value && test0 != "cats" && test0 != "trees" && test0 != "pomeranians" && test0 != "goldfish" {
				isMatchy = false
			}
			if test1 == v.test && result1 != v.value && test1 != "cats" && test1 != "trees" && test1 != "pomeranians" && test1 != "goldfish" {
				isMatchy = false
			}
			if test2 == v.test && result2 != v.value && test2 != "cats" && test2 != "trees" && test2 != "pomeranians" && test2 != "goldfish" {
				isMatchy = false
			}

			if test0 == v.test && result0 <= v.value && (test0 == "cats" || test0 == "trees") {
				isMatchy = false
			}
			if test1 == v.test && result1 <= v.value && (test1 == "cats" || test1 == "trees") {
				isMatchy = false
			}
			if test2 == v.test && result2 <= v.value && (test2 == "cats" || test2 == "trees") {
				isMatchy = false
			}

			if test0 == v.test && result0 >= v.value && (test0 == "pomeranians" || test0 == "goldfish") {
				isMatchy = false
			}
			if test1 == v.test && result1 >= v.value && (test1 == "pomeranians" || test1 == "goldfish") {
				isMatchy = false
			}
			if test2 == v.test && result2 >= v.value && (test2 == "pomeranians" || test2 == "goldfish") {
				isMatchy = false
			}
